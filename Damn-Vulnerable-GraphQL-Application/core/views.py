...
from core import security, helpers, middleware
from core.directives import *
from core.models import Owner, Paste, User, Audit
from core.view_override import OverriddenView, GeventSubscriptionServerCustom, format_custom_error
...
class SystemUpdateObject(graphene.ObjectType):
    message = graphene.String()


class SystemHealthObject(graphene.ObjectType):
    load = graphene.String()


class SystemDebugObject(graphene.ObjectType):
    output = graphene.String()


class SystemDiagnosticsObject(graphene.ObjectType):
    output = graphene.String()
...
# ----------------- Qevlar Dummy Objects -----------------
class DummyPaste(graphene.ObjectType):
    id = graphene.Int()
    title = graphene.String()
    content = graphene.String()
    owner = graphene.Field(lambda: DummyUser)


class DummyUser(graphene.ObjectType):
    id = graphene.Int()
    username = graphene.String()
    last_paste = graphene.Field(DummyPaste)


dummy_user = DummyUser(id=1, username="TestUser")
dummy_paste = DummyPaste(id=1, title="TestPaste", content="Test content", owner=dummy_user)
dummy_user.last_paste = dummy_paste
...
class Query(graphene.ObjectType):
    ...
    paste = graphene.Field(PasteObject, id=graphene.Int(), title=graphene.String())
    system_update = graphene.Field(SystemUpdateObject)
    system_health = graphene.Field(SystemHealthObject)
    system_debug = graphene.Field(SystemDebugObject, arg=graphene.String())
    system_diagnostics = graphene.Field(SystemDiagnosticsObject, username=graphene.String(), password=graphene.String(), cmd=graphene.String())
    ...
    # Qevlar dummy fields
    dummy_user = graphene.Field(DummyUser)
    dummy_paste = graphene.Field(DummyPaste)
    all_dummy_pastes = graphene.List(DummyPaste)

    # Resolvers for dummy fields
    def resolve_dummy_user(self, info):
        return dummy_user

    def resolve_dummy_paste(self, info):
        return dummy_paste

    def resolve_all_dummy_pastes(self, info):
        return [dummy_paste for _ in range(50)]

    # Real resolvers
    def resolve_me(self, info, token):
        Audit.create_audit_entry(info)
        identity = get_identity(token)
        if info.context.json is None:
            raise GraphQLError("JSON payload was not found.")
        info.context.json['identity'] = identity
        query = UserObject.get_query(info)
        return query.filter_by(username=identity).first()
    ...
    def resolve_search(self, info, keyword=None):
        ...
        if keyword:
            ...
            items.extend(Paste.query.filter(Paste.title.like(search)))
            items.extend(User.query.filter(User.username.like(search)))
        else:
            items.extend(Paste.query.all())
            items.extend(User.query.all())
        return items

    def resolve_pastes(self, info, public=False, limit=1000, filter=None):
        Audit.create_audit_entry(info)
        query = PasteObject.get_query(info).filter_by(public=public, burn=False)
        if filter:
            query = query.filter(text("title = '%s' or content = '%s'" % (filter, filter)))
        return query.order_by(Paste.id.desc()).limit(limit)
    ...
    def resolve_system_update(self, info):
        Audit.create_audit_entry(info)
        return SystemUpdateObject(message="no updates available")

    def resolve_system_health(self, info):
        Audit.create_audit_entry(info)
        load = helpers.run_cmd("uptime | awk -F': ' '{print $2}' | awk -F',' '{print $1}'")
        return SystemHealthObject(load=load)

    def resolve_system_debug(self, info, arg=None):
        Audit.create_audit_entry(info)
        output = helpers.run_cmd(f'ps {arg}') if arg else helpers.run_cmd('ps')
        return SystemDebugObject(output=output)
    ...
    def resolve_system_diagnostics(self, info, username, password, cmd='whoami'):
        ...
        if res and security.allowed_cmds(cmd):
            output = helpers.run_cmd(cmd)
        return SystemDiagnosticsObject(output=output if res else msg)

    def resolve_users(self, info, id=None):
        Audit.create_audit_entry(info)
        query = UserObject.get_query(info)
        return query.filter_by(id=id) if id else query
...
def get_difficulty():
    level = 'easy' if helpers.is_level_easy() else 'hard'
...
def set_difficulty():
    ...
    if mode_header:
        helpers.set_mode('hard' if mode_header == 'Expert' else 'easy')
